...
#include "wise_enum_detail.hpp"
#include "wise_enum_generated.hpp"
...
template <class T> constexpr string_type to_string(T t) {
...
// for C++14 and on
template <class T> struct enumerators {
...
template <class T>
constexpr decltype(wise_enum_detail_array(
    detail::Tag<T>{})) enumerators<T>::range;

template <class T> constexpr std::size_t enumerators<T>::size;

#if __cplusplus >= 201402
template <class T> constexpr auto &range = enumerators<T>::range;

template <class T> constexpr std::size_t size = enumerators<T>::size;
#endif

// A type trait; this allows checking if a type is a wise_enum in generic code
template <class T> using is_wise_enum = detail::is_wise_enum<T>;
